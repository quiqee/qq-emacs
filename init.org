...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs.d/bin/packages.el
  ...
  (mapcar (lambda (package)
          ...
          '(
            ...
            (pragmatapro-lig :fetcher file :path "~/SeaDrive/My Libraries/Personal/Fonts/pragmatapro-lig.el")
...
#+begin_src emacs-lisp
  (setq qq/shell-prog "/usr/bin/bash")
#+end_src
...
#+BEGIN_SRC emacs-lisp
     (defvar best-gc-cons-threshold (* 100 1024 1024) "Best default gc threshold value (100 MB). Shouldn't be too big.")
#+END_SRC

*** Emacs Garbage Collector
   Once startup complete decrease threshold to 100 MB
...
#+BEGIN_SRC emacs-lisp
  redisplay-dont-pause t ;; Don't pause during redisplay
  ;; Optimize redisplay performance
  ...
  ;; Reduce work in visible area
  cursor-in-non-selected-windows nil
  highlight-nonselected-windows nil

  ;; For more complex buffers
  idle-update-delay 1.0 ;; Default is shorter
#+END_SRC
...
*** Eldoc
#+BEGIN_SRC emacs-lisp
  (setq eldoc-idle-delay 1.0) ;; Increase eldoc delay
#+END_SRC
...
*** Compilation mode
   Set vertical line mode
#+BEGIN_SRC emacs-lisp
  ;; Enable word wrapping in compilation buffers
...
#+begin_src emacs-lisp
  ...
  (setq aidermacs-default-model "r1")
...
#+BEGIN_SRC emacs-lisp
  (require 'blamer)
  (custom-set-variables
   '(blamer-idle-time 1.0)   ;; Increase delay before showing blame info
   '(blamer-min-offset 70))  ;; Only show blame for lines with this much space
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Reduce frequency of idle timers
  (setq which-key-idle-delay 2.0)  ;; Increase from 1.0s
  (setq which-func-update-interval 3.0)  ;; Increase delay for which-function updates
...
** Compilation Buffer Map Bindings
   Generate binding using table from  [[Compilation Buffer Binding][here]]
