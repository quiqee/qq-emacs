...
*** Tangling Function
   ...
   Tangle function below uses [[https://github.com/jwiegley/emacs-async][async.el]] to open and tangle an org file
         ...
         (let ((init-tangle-start-time (current-time))
           ...
           (async-start
            ...
            (unless qq/show-async-tangle-results
              `(lambda (result)
                 (if result
                     (alert "SUCCESS: %s successfully tangled. (%.3fs)"
                            ,(file-name-nondirectory file)
                            (float-time (time-subtract (current-time)
                                                       ',init-tangle-start-time)))
                   (alert "ERROR: %s tangle failed." ,(file-name-nondirectory file)))))))))
     (setq qq/show-async-tangle-results 1)
   #+END_SRC
