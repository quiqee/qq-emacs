...
- **[[*Kotlin Mode][kotlin-mode]]**: Kotlin language support for KMP development
- **[[*Swift Mode][swift-mode]]**: Swift language support for iOS development
- **[[*LSP SourceKit][lsp-sourcekit]]**: SourceKit-LSP client for Swift language server
- **[[*Groovy Mode][groovy-mode]]**: Groovy/Gradle build script support
- **[[*Gradle Mode][gradle-mode]]**: Gradle build system integration with task execution
- **[[*KMP Development][kmp-dev]]**: Transient menu for Kotlin Multiplatform development (Android/iOS builds, tests)
- **[[*Typst TS Mode][typst-ts-mode]]**: Tree-sitter powered Typst editing and compilation
...
- **[[*Xterm Color][xterm-color]]**: Terminal escape sequence handling and ANSI colors for compilation progress bars
...
- **[[*Valign][valign]]**: Pixel-perfect visual alignment for org tables with mixed fonts
...
- **[[*Ox Typst][ox-typst]]**: Org export backend for Typst PDF generation
...
#+NAME: gen-keys
#+BEGIN_SRC emacs-lisp :var tname=1 :var mode="leader" :results output silent :tangle no :exports none :colnames yes
(dolist (row tname)
...
#+NAME: gen-use-pkg
#+BEGIN_SRC emacs-lisp :var package="example" :var after="" :var straight="" :var strght_gh="" :var demand="" :var defer="" :var hook="" :var mode="" :var init="" :var firsttag="config" :results value silent :tangle no :exports none
(let ((header (format "(use-package %s" package))
      ...
      (mode-line (if (string= mode "") "" (format "\n  :mode %s" mode)))
      (init-line (if (string= init "") "" (format "\n  :init\n  %s" init)))
      (config-line (format "\n  :%s" firsttag)))
  (concat header straight-line after-line demand-line defer-line hook-line mode-line init-line config-line))
#+END_SRC

#+NAME: gen-pkg-end
#+BEGIN_SRC emacs-lisp :var close_paren=")" :results value silent :tangle no :exports none
close_paren
#+END_SRC

** Bin Scripts
Batch scripts used for tangling and package management outside of interactive Emacs.

#+BEGIN_SRC emacs-lisp :tangle ~/.emacs.d/bin/compile.el :exports none
;;; compile.el --- Tangle init.org to init.el  -*- lexical-binding: t; -*-
;; Run with: emacs --script ~/.emacs.d/bin/compile.el
(setq org-confirm-babel-evaluate nil)
(require 'ob-tangle)
(let ((init-org (expand-file-name "init.org" user-emacs-directory)))
  (message "Tangling %s..." init-org)
...
(defvar qq/org-base-directory nil
  "Base directory for org files, set per-site.")
#+END_SRC

*** Linux Work Desktop (eagle, Legion-U)
#+NAME: linux-work-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
(setq qq/org-base-directory "~/ownCloud/Personal/Org")
(setq qq/org-roam-base-directory (concat qq/org-base-directory "/RoamNotes"))
...
;; Ensure PragmataPro Liga is used for its PUA ligature codepoints
(set-fontset-font t '(#XE000 . #XF8FF) (font-spec :family "PragmataPro Liga") nil 'prepend)
(set-fontset-font t '(#X100000 . #X10FFFF) (font-spec :family "PragmataPro Liga") nil 'prepend)
;; Apply modeline font height
(set-face-attribute 'mode-line nil :height qq/modeline-font-height)
(set-face-attribute 'mode-line-inactive nil :height qq/modeline-font-height)
...
;; Java 21 for KMP development
(setenv "JAVA_HOME" "/opt/homebrew/opt/openjdk@21")
...
#+NAME: default-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
(setq qq/org-base-directory "~/Documents/Org-files")
(setq qq/org-roam-base-directory (concat qq/org-base-directory "/RoamNotes"))
...
** Community Config Best Practices
Settings collected from popular Emacs configurations (Doom, Purcell, Prot, Centaur). Source:
[[https://emacsredux.com/blog/2026/04/07/stealing-from-the-best-emacs-configs/][Stealing from the Best Emacs Configs]] by Bozhidar Batsov. → [[*Community Config Best Practices Code][Generated Code]]
#+NAME: community-config-best-practices
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; --- Performance ---
;; Fully disable bidirectional text scanning (left-to-right users only).
;; Gives noticeable redisplay speedups in large files.
(setq-default bidi-display-reordering 'left-to-right
              bidi-paragraph-direction 'left-to-right)
(setq bidi-inhibit-bpa t)

;; Defer syntax highlighting while typing — eliminates micro-stutters.
(setq redisplay-skip-fontification-on-input t)

;; Modern LSP servers (e.g. rust-analyzer) send large responses.
;; 4 MB avoids repeated small reads; default is 64 KB.
(setq read-process-output-max (* 4 1024 1024))

;; --- Kill Ring & Clipboard ---
;; Don't clutter the kill ring with duplicates.
(setq kill-do-not-save-duplicates t)

;; Persist kill ring across sessions via savehist.
(with-eval-after-load 'savehist
  (add-to-list 'savehist-additional-variables 'kill-ring))

;; Strip text properties before saving kill ring to avoid bloating the file.
...
;; Auto-chmod files with a shebang line after saving.
(add-hook 'after-save-hook
          #'executable-make-buffer-file-executable-if-script-p)

;; Use string syntax in re-builder — no double-escaping needed.
(setq reb-re-syntax 'string)

;; Prevent find-file-at-point from pinging hostnames (causes multi-second hangs).
(setq ffap-machine-p-known 'reject)

;; --- Window Management ---
;; Resize all windows proportionally when splitting — keeps layouts balanced.
(setq window-combination-resize t)

;; Make C-x 1 toggle: zoom to single window or restore previous layout.
(winner-mode +1)
(defun qq/toggle-delete-other-windows ()
...
;; --- Miscellaneous ---
;; Allow continuous mark popping with C-SPC after initial C-u C-SPC.
(setq set-mark-command-repeat-pop t)

;; Recenter buffer after save-place restores cursor position.
(advice-add 'save-place-find-file-hook :after
            (lambda (&rest _)
              (when buffer-file-name (ignore-errors (recenter)))))

;; Auto-focus help windows opened via C-h f / C-h v.
...
*** KMP Development Leader Key Bindings
Kotlin Multiplatform development keybindings. → [[*General Package][Generated Code]]
#+NAME: kmp-keys
| Key | Command      | Description                  |
|-----+--------------+------------------------------|
| k   | qq/kmp-menu  | Open KMP development menu    |
...
| x c | org-roam-capture   | org-capture for new or existing node |
...
| C-w C-e | qq/toggle-delete-other-windows   | toggle: zoom window or restore previous layout |
...
*** Compilation Mode Keybindings Table
Compilation mode navigation and control keybindings. → [[*Xterm Color Package][Generated Code]]
#+NAME: compilation-mode-keys
| Key   | Command                    | Description                           |
|-------+----------------------------+---------------------------------------|
...
| <C-return>    | vertico-exit-input               | Submit exact input text        |
...
*** Typst TS Mode Keybindings Table
Typst mode keybindings for compilation and preview. → [[*Typst TS Mode Package][Generated Code]]
#+NAME: typst-ts-mode-keys
| Key   | Command                   | Description             |
|-------+---------------------------+-------------------------|
| C-c p | qq/typst-save-and-preview | Save and preview as PDF |
...
;; Configure Kotlin Language Server for KMP development
(with-eval-after-load 'lsp-mode
...
(kotlin-mode . qq/lsp-deferred-if-supported)
(swift-mode . qq/lsp-deferred-if-supported)
...
(add-to-list 'yas-snippet-dirs (concat qq/cache-directory "snippets/"))
...
***** Kotlin Mode
Kotlin language support for KMP (Kotlin Multiplatform) development targeting iOS and Android. → [[*Kotlin Mode Package][Generated Code]]

****** Kotlin Mode Configuration Block
#+NAME: kotlin-mode-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Associate kotlin-mode with Kotlin files
...
***** Swift Mode
Swift language support for iOS development and KMP bridging code. → [[*Swift Mode Package][Generated Code]]

****** Swift Mode Configuration Block
#+NAME: swift-mode-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Associate swift-mode with Swift files
...
***** LSP SourceKit
SourceKit-LSP client for Swift language server support. → [[*LSP SourceKit Package][Generated Code]]

****** LSP SourceKit Configuration Block
#+NAME: lsp-sourcekit-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Configure sourcekit-lsp executable path
(setq lsp-sourcekit-executable
      "/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/sourcekit-lsp")
#+END_SRC

***** Groovy Mode
Groovy language support for Gradle build scripts used in KMP projects. → [[*Groovy Mode Package][Generated Code]]

****** Groovy Mode Configuration Block
#+NAME: groovy-mode-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Associate groovy-mode with Gradle and Groovy files
...
***** Gradle Mode
Gradle build system integration for running Gradle tasks directly from Emacs. Essential for KMP (Kotlin Multiplatform) development workflow. → [[*Gradle Mode Package][Generated Code]]

****** Gradle Mode Configuration Block
#+NAME: gradle-mode-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Enable gradle-mode for Kotlin and Groovy files
...
;; Find gradlew from project root using projectile
(defun qq/gradle-find-gradlew ()
  "Find gradlew executable in project root."
  (when-let ((root (projectile-project-root)))
    (let ((gradlew (expand-file-name "gradlew" root)))
      (when (file-executable-p gradlew)
        gradlew))))

(setq gradle-executable-path #'qq/gradle-find-gradlew)
#+END_SRC

***** KMP Development
Kotlin Multiplatform (KMP) development utilities with transient menu for common build tasks. Provides quick access to Gradle commands for Android, iOS, and shared module development. → [[*KMP Development Code][Generated Code]]

****** KMP Development Configuration Block
#+NAME: kmp-dev-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; KMP Gradle helper function
(defun qq/kmp-gradle (task)
  "Run Gradle TASK in project root using gradlew."
  (interactive "sGradle task: ")
  (let ((default-directory (or (projectile-project-root) default-directory)))
    (compile (concat "./gradlew " task))))

;; Individual task functions for transient
(defun qq/kmp-build () (interactive) (qq/kmp-gradle "build"))
(defun qq/kmp-clean () (interactive) (qq/kmp-gradle "clean"))
(defun qq/kmp-clean-build () (interactive) (qq/kmp-gradle "clean build"))
(defun qq/kmp-android-debug () (interactive) (qq/kmp-gradle "androidApp:installDebug"))
(defun qq/kmp-android-release () (interactive) (qq/kmp-gradle "androidApp:assembleRelease"))
(defun qq/kmp-android-assemble () (interactive) (qq/kmp-gradle "androidApp:assembleDebug"))
(defun qq/kmp-android-run-emulator ()
  (interactive)
  (let ((default-directory (or (projectile-project-root) default-directory))
        (emulator "/opt/homebrew/share/android-commandlinetools/emulator/emulator")
        (adb "/opt/homebrew/share/android-commandlinetools/platform-tools/adb"))
    ;; Start emulator in separate process if not already running
    (unless (= 0 (shell-command (format "%s devices | grep -q emulator" adb)))
      (start-process "android-emulator" nil emulator "-avd"
                     (string-trim (shell-command-to-string (format "%s -list-avds | head -1" emulator))))
      (sleep-for 5))
    (compile (format "./gradlew androidApp:installDebug && %s shell am start -n com.rahl/.MainActivity" adb))))
(defun qq/kmp-ios-build ()
  (interactive)
  (let ((default-directory (or (projectile-project-root) default-directory)))
    (compile "cd iosApp && xcodegen generate && cd .. && ./gradlew :composeApp:linkDebugFrameworkIosSimulatorArm64")))
(defun qq/kmp-ios-arm ()
  (interactive)
  (let ((default-directory (or (projectile-project-root) default-directory)))
    (compile "cd iosApp && xcodegen generate && cd .. && ./gradlew :composeApp:linkDebugFrameworkIosArm64")))
(defun qq/kmp-ios-run-simulator (&optional simulator-name)
  (interactive)
  (let* ((name (or simulator-name "iPhone 17 Pro Max"))
         (default-directory (or (projectile-project-root) default-directory)))
    (compile (format "xcodebuild -project iosApp/iosApp.xcodeproj -scheme iosApp -sdk iphonesimulator -destination 'platform=iOS Simulator,name=%s' build && xcrun simctl boot '%s' 2>/dev/null; open -a Simulator && xcrun simctl install booted \"$(find ~/Library/Developer/Xcode/DerivedData/iosApp-*/Build/Products/Debug-iphonesimulator -name 'iosApp.app' -type d | head -1)\" && xcrun simctl launch booted com.rahl.app" name name))))
(defun qq/kmp-ios-run-ipad ()
  (interactive)
  (qq/kmp-ios-run-simulator "iPad Air 13-inch (M3)"))
(defun qq/kmp-android-logcat ()
  (interactive)
  (let ((adb "/opt/homebrew/share/android-commandlinetools/platform-tools/adb"))
    (async-shell-command (format "%s logcat -v time | grep -iE '(rahl|kermit)'" adb) "*android-logcat*")))
(defun qq/kmp-ios-log ()
(interactive)
(async-shell-command "xcrun simctl spawn booted log stream --level debug --predicate 'process == \"iosApp\"' 2>&1 | grep --line-buffered '🔵' | sed 's/^\\([^ ]* [^ ]*\\) .* 🔵 (rahl) /\\1 /'" "*ios-log*"))
(defun qq/kmp-test-all () (interactive) (qq/kmp-gradle "test"))
(defun qq/kmp-test-shared () (interactive) (qq/kmp-gradle "shared:test"))
(defun qq/kmp-test-android () (interactive) (qq/kmp-gradle "androidApp:test"))
(defun qq/kmp-dependencies () (interactive) (qq/kmp-gradle "dependencies"))
(defun qq/kmp-tasks () (interactive) (qq/kmp-gradle "tasks"))
(defun qq/kmp-sync () (interactive) (qq/kmp-gradle "kotlinUpgradeYarnLock"))

;; Transient menu for KMP development
(with-eval-after-load 'transient
  (transient-define-prefix qq/kmp-menu ()
    "Kotlin Multiplatform Development Menu"
    ["Build"
     ("b" "Build All" qq/kmp-build)
     ("c" "Clean" qq/kmp-clean)
     ("C" "Clean + Build" qq/kmp-clean-build)]
    ["Android"
     ("a" "Install Debug" qq/kmp-android-debug)
     ("A" "Build Debug" qq/kmp-android-assemble)
     ("e" "Build Release" qq/kmp-android-release)
     ("R" "Run Emulator" qq/kmp-android-run-emulator)
     ("l" "Logcat" qq/kmp-android-logcat)]
    ["iOS"
     ("i" "Build Simulator" qq/kmp-ios-build)
     ("I" "Build ARM64" qq/kmp-ios-arm)
     ("r" "Run iPhone" qq/kmp-ios-run-simulator)
     ("p" "Run iPad" qq/kmp-ios-run-ipad)
     ("L" "Log Stream" qq/kmp-ios-log)]
    ["Test"
     ("t" "Test All" qq/kmp-test-all)
     ("s" "Test Shared" qq/kmp-test-shared)
     ("T" "Test Android" qq/kmp-test-android)]
    ["Info"
     ("d" "Dependencies" qq/kmp-dependencies)
     ("?" "List Tasks" qq/kmp-tasks)
     ("g" "Run Task..." qq/kmp-gradle)]))
#+END_SRC

***** Typst TS Mode
Tree-sitter powered Typst editing with syntax highlighting, compilation, and preview support. Requires =typst= binary (=brew install typst=). → [[*Typst TS Mode Package][Generated Code]]

****** Typst TS Mode Configuration Block
#+NAME: typst-ts-mode-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Associate typst-ts-mode with Typst files
(add-to-list 'auto-mode-alist '("\\.typ\\'" . typst-ts-mode))

;; Set typst executable path
(setq typst-ts-mode-executable-location (executable-find "typst"))

;; Hide compilation buffer unless there is an error
(defun qq/typst-compilation-finish (buf msg)
  "Delete compilation window if compilation succeeded.
BUF is the compilation buffer, MSG is the status message."
  (when (and (string-match-p "finished" msg)
             (get-buffer-window buf))
    (delete-window (get-buffer-window buf))))

(add-hook 'typst-ts-compile-after-compilation-hook #'qq/typst-compilation-finish)

;; Set preview function to open PDF in Emacs buffer
(setq typst-ts-mode-preview-function #'find-file)

(defun qq/typst-save-and-preview ()
  "Save, compile, and open the resulting PDF in the current window."
  (interactive)
  (save-buffer)
...
**** Xterm Color
Xterm-color provides proper terminal escape sequence handling for compilation buffers, enabling progress bars to update in place. → [[*Xterm Color Package][Generated Code]]

***** Xterm Color Configuration Block
#+NAME: xterm-color-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Handle ANSI colors and carriage returns in compilation mode
(setq compilation-environment '("TERM=xterm-256color"))

;; Remove old xterm-color advice if present
(advice-remove 'compilation-filter #'qq/advice-compilation-filter)

;; Custom carriage return handler for compilation buffers
;; comint-carriage-motion doesn't work reliably, so we handle \r manually
(defun qq/handle-carriage-returns (start end)
  "Handle carriage returns by deleting from line start to \\r.
This makes progress bars update in place like in a terminal."
  (save-excursion
    (goto-char start)
    (while (search-forward "\r" end t)
      ;; Found a \r - delete from beginning of line to here (the \r itself)
      (let ((bol (save-excursion (goto-char (match-beginning 0))
                                  (line-beginning-position))))
        (delete-region bol (point))))))

;; Define a named function for the compilation filter hook
(defun qq/compilation-filter-hook ()
  "Handle carriage returns and ANSI colors in compilation output."
  (let ((inhibit-read-only t))
    ;; First handle carriage returns for progress bars
    (qq/handle-carriage-returns compilation-filter-start (point))
    ;; Then apply ANSI color codes
    (ansi-color-apply-on-region compilation-filter-start (point))))

;; Remove any existing hook before adding
(remove-hook 'compilation-filter-hook #'qq/compilation-filter-hook)
(add-hook 'compilation-filter-hook #'qq/compilation-filter-hook)
...
;; Use vterm for compilation - proper terminal emulation for progress bars
;; This overrides compilation-start to use vterm terminal emulation
;; Inspired by: https://codeberg.org/akib/emacs-eat/issues/33
(defvar qq/vterm-compilation-excluded-modes '(typst-ts-compilation-mode)
  "List of modes that should use standard compilation instead of vterm.
These modes require major mode changes that vterm doesn't support.")

(defun qq/vterm-compilation-start (command &optional mode name-function
                                            highlight-regexp continue)
  "Run COMMAND in vterm with compilation-minor-mode.
This provides full terminal emulation for progress bars and colors.
Falls back to standard compilation for modes in `qq/vterm-compilation-excluded-modes'."
  ;; Fall back to standard compilation for excluded modes
  (if (memq mode qq/vterm-compilation-excluded-modes)
      (progn
        (advice-remove #'compilation-start #'qq/vterm-compilation-start)
        (unwind-protect
            (compilation-start command mode name-function highlight-regexp continue)
          (advice-add #'compilation-start :override #'qq/vterm-compilation-start)))
    ;; Use vterm for normal compilation
    (require 'vterm)
    (let ((name-of-mode "compilation")
      ...
      (if (or (not mode) (eq mode t))
        ...
        (setq name-of-mode (replace-regexp-in-string "-mode\\'" ""
                                                     (symbol-name mode))))
      (setq outbuf (get-buffer-create
                    (compilation-buffer-name name-of-mode mode name-function)))
      ;; Kill existing process if any
      (let ((proc (get-buffer-process outbuf)))
        (when proc
          (delete-process proc)))
      ;; Kill and recreate buffer to avoid vterm major mode change error
      (kill-buffer outbuf)
      (setq outbuf (get-buffer-create
                    (compilation-buffer-name name-of-mode mode name-function)))
      (with-current-buffer outbuf
        (setq default-directory dir)
        (let ((vterm-shell (format "%s -lc %s" shell-file-name
...
**** TRAMP
Remote file access with BusyBox compatibility for embedded hosts. → [[*TRAMP Package][Generated Code]]

***** TRAMP Configuration Block
#+NAME: tramp-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; TRAMP debug logging - set to 6+ for detailed debug, 1 for normal use
;; (setq tramp-verbose 10)

;; BusyBox-compatible TRAMP settings for eagle-*.local hosts
;; These hosts run GNU bash but BusyBox userland utilities.
;; The "ls" property is the command name that tramp-get-ls-command returns.
;; Tramp appends switches separately, so this must be just the binary name.
;; Setting "stat" to nil prevents tramp from using stat (missing on BusyBox).
(dolist (prop '(("ls" "ls")
               ("stat" nil)
               ("remote-shell" "/bin/bash")))
  (add-to-list 'tramp-connection-properties
               (cons "/.*:eagle-.*\\.local:" prop)))

;; Ensure /bin is in remote path for BusyBox systems
(add-to-list 'tramp-remote-path "/bin")

;; Connection-local variables for TRAMP buffers.
;; On macOS insert-directory-program is /opt/homebrew/bin/gls which doesn't
;; exist on remote hosts. Override to "ls" for all TRAMP connections.
;; Also use simple dired-listing-switches since BusyBox ls doesn't support
;; GNU flags like --group-directories-first, --time-style, or -v.
;; The dired-before-readin-hook approach doesn't work because insert-directory
;; is called with the global switches before the hook fires.
...
(doom-modeline-def-modeline 'qq-simple-line
  '(bar modals window-number buffer-position matches buffer-info remote-host minor-modes)
...
(when (display-graphic-p)
  ...
  ;; Ensure PragmataPro Liga PUA codepoints are not overridden by emoji fonts
  (set-fontset-font t '(#XE000 . #XF8FF) (font-spec :family "PragmataPro Liga") nil 'prepend)
  (set-fontset-font t '(#X100000 . #X10FFFF) (font-spec :family "PragmataPro Liga") nil 'prepend)
...
;; Org directory and agenda files (set per-site via qq/org-base-directory)
(when qq/org-base-directory
  (setq org-directory qq/org-base-directory)
  (setq org-agenda-files (list (concat qq/org-base-directory "/Agenda"))))
...
;; Images
(setq org-image-actual-width '(800))         ; Default image width
...
(defun qq/save-tangle-reload-config ()
  "Save init.org, tangle via a batch subprocess, and reload init.el when done."
  (interactive)
  (when (string-match "init\\.org$" (buffer-file-name))
    (save-buffer)
    (message "Tangling config (async)...")
    (let* ((compile-el (expand-file-name "bin/compile.el" user-emacs-directory))
           (cmd (if (file-exists-p compile-el)
                    (list "emacs" "--script" compile-el)
                  (list "emacs" "--batch"
      ...
      (make-process
       ...
       :sentinel (lambda (proc event)
                   (if (string-prefix-p "finished" event)
                     ...
                     (message "Tangle failed — check *qq-tangle* buffer")))))))

(defvar qq/babel-tangling-p nil "Non-nil while org-babel tangle is in progress.")
(add-hook 'org-babel-pre-tangle-hook  (lambda () (setq qq/babel-tangling-p t)))
(add-hook 'org-babel-post-tangle-hook (lambda () (setq qq/babel-tangling-p nil)))

(add-hook 'org-mode-hook  (lambda ()
                            (electric-indent-local-mode -1) ;; disable electric indentation
                            (unless qq/babel-tangling-p
...
(defvar qq/babel-languages
  '(shell python js plantuml sql org dot rust typescript
    http restclient java C perl ruby clojure lua haskell latex)
...
;; Also load babel languages before export (export doesn't trigger the advice)
(advice-add 'org-export-dispatch :before #'qq/load-babel-languages-on-demand)
...
*** Valign
Pixel-perfect visual alignment for org-mode tables containing mixed fonts such as LaTeX math, CJK characters, or images. Replaces character-count-based alignment with pixel-width measurement to keep columns visually aligned regardless of font mixing. → [[*Valign Package][Generated Code]]

**** Valign Configuration Block
#+NAME: valign-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Enable valign-mode for visual table alignment in org buffers
;; Only enable on smaller files to avoid performance issues
;; (defun qq/maybe-enable-valign ()
;;   (when (< (buffer-size) 100000)
;;     (valign-mode 1)))
;; (add-hook 'org-mode-hook #'qq/maybe-enable-valign)
#+END_SRC

*** Ox Typst
Org export backend for Typst, enabling PDF generation from org files via Typst. Export with =C-c C-e T p= (Typst to PDF). Requires =typst= binary (=brew install typst=). → [[*Ox Typst Package][Generated Code]]

**** Ox Typst Configuration Block
#+NAME: ox-typst-config
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; ox-typst is loaded on demand via org export dispatcher
(require 'ox-typst)

;; Fix ox-typst bug: handle nil :input-file gracefully
(defun qq/ox-typst-fix-nil-input-file (orig-fun typst-file &optional no-input)
  "Advice to handle nil TYPST-FILE in `org-typst--generate-command'."
  (funcall orig-fun (or typst-file (buffer-file-name) "untitled.typ") no-input))

(advice-add 'org-typst--generate-command :around #'qq/ox-typst-fix-nil-input-file)
#+END_SRC
...
;; Initialize pdf-tools server (epdfinfo)
...
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Enable all PragmataPro ligatures globally (prog-mode, text-mode, etc.)
...
#+BEGIN_SRC emacs-lisp :tangle no :exports none
;; Official PragmataPro prettify-symbols configuration (v0.830)
;; Uses prettify-symbols-mode to compose operator sequences into
;; PragmataPro PUA glyphs while preserving per-character cursor movement.
...
*** Community Config Best Practices Code
Settings from popular Emacs configurations. → [[*Community Config Best Practices][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<community-config-best-practices>>
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="evil-collection", after="evil", firsttag="init")>>
...
*** Kotlin Mode Package
Kotlin language support for KMP (Kotlin Multiplatform) development. → [[*Kotlin Mode][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="kotlin-mode")>>
<<kotlin-mode-config>>
<<gen-pkg-end()>>
#+END_SRC

*** Swift Mode Package
Swift language support for iOS development and KMP bridging. → [[*Swift Mode][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="swift-mode")>>
<<swift-mode-config>>
<<gen-pkg-end()>>
#+END_SRC

*** LSP SourceKit Package
SourceKit-LSP client for Swift language server. → [[*LSP SourceKit][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="lsp-sourcekit", after="lsp-mode")>>
<<lsp-sourcekit-config>>
<<gen-pkg-end()>>
#+END_SRC

*** Groovy Mode Package
Groovy/Gradle build script support for KMP projects. → [[*Groovy Mode][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="groovy-mode")>>
<<groovy-mode-config>>
<<gen-pkg-end()>>
#+END_SRC

*** Gradle Mode Package
Gradle build system integration for KMP projects. → [[*Gradle Mode][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="gradle-mode")>>
<<gradle-mode-config>>
<<gen-pkg-end()>>
#+END_SRC

*** KMP Development Code
KMP transient menu for Kotlin Multiplatform development. → [[*KMP Development][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
;; KMP Development utilities (no package, just config)
<<kmp-dev-config>>
#+END_SRC
...
*** Xterm Color Package
Terminal color handling with proper escape sequence support for compilation. → [[*Xterm Color][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="xterm-color")>>
<<xterm-color-config>>
;; Add compilation-mode keybindings using general.el
(with-eval-after-load 'general
  (general-def 'normal compilation-mode-map
    <<gen-keys(mode="local-mode", tname=compilation-mode-keys)>>))
<<gen-pkg-end()>>
#+END_SRC

*** Fancy Compilation Package (DISABLED)
Enhanced compilation output with progress display. Disabled because it interferes with carriage return handling for progress bars.
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent :tangle no
...
(qq/leader-keys
  ...
  <<gen-keys(mode="leader", tname=kmp-keys)>>
...
** Typst TS Mode Package
Tree-sitter powered Typst editing and compilation. → [[*Typst TS Mode][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="typst-ts-mode", strght_gh="Ziqi-Yang/typst-ts-mode")>>
<<typst-ts-mode-config>>
:general
(:states 'normal
         :keymaps 'typst-ts-mode-map
         <<gen-keys(mode="local-mode", tname=typst-ts-mode-keys)>>)
<<gen-pkg-end()>>
#+END_SRC
...
*** TRAMP Package
Remote file access with BusyBox compatibility for embedded hosts. → [[*TRAMP][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
(use-package tramp
...
*** Valign Package
Pixel-perfect visual alignment for org tables with mixed fonts. → [[*Valign][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="valign")>>
<<valign-config>>
<<gen-pkg-end()>>
#+END_SRC

*** Ox Typst Package
Org export backend for Typst PDF generation. → [[*Ox Typst][Configuration]]
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="ox-typst", strght_gh="jmpunkt/ox-typst")>>
<<ox-typst-config>>
<<gen-pkg-end()>>
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
<<gen-use-pkg(package="pdf-tools", mode="(\"\\\\.pdf\\\\'\" . pdf-view-mode)")>>
<<pdf-tools-config>>
<<gen-pkg-end()>>
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :noweb yes :exports none :results silent
;; <<gen-use-pkg(package="pragmatapro-lig", after="ligature", strght_gh="lumiknit/emacs-pragmatapro-ligatures")>>
