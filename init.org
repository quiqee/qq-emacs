...
*** Keybindings
    ...
    :END:
   The table below will be used as the source for generating [[Basic Bindings]]
...
** Evil
   ...
   I don't like the default behaviour for * and # commands. It only search
   subword not the whole word. I.e, if a cursor is in 'mamb' for keyword
   mamb_ctrl_scaleConversion. Then only 'mamb' will be search. This setting
   will mimic exactly the behaviour in Vim:
...
*** Cscope Bindings
   Binding table is taken from [[CScope][here]]. Before binding is generated, prefix code is
