...
<<<<<<< Updated upstream
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     ;                (expand-file-name "~/LocalDocuments/git/qq-emacs/init.org"))
     ;         (equal (buffer-file-name)
...
=======
   ...
   asynchronously. This is how I tangle init.org (and rc.org) directly to
   init.el on save, since it’s long enough that a synchronous tangle causes
   a pretty annoynig lag.

    - If qq/asyncbabel-tangle-decrypt is set to t in the [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Specifying-File-Variables.html#Specifying-File-Variables][file variables]], org
      entries are decrypted first. Mostly useful if you do something like
      keep sensitive configuration files in an org file.
    - If qq/show-async-tangle-results is t, don’t dispose of async emacs process
      buffers. This is useful for debugging errors.

   #+BEGIN_SRC emacs-lisp
     (after 'org
       (defvar qq/show-async-tangle-results nil
         "Keep *emacs* async buffers around for later inspection.")
       (defvar qq/async-babel-tangle-decrypt nil
         "Decrypt org entries before tangling. Probably most useful as a file-local variable.")

       (defun qq/async-babel-tangle (&optional decrypt)
...
*** Common
   ...
   #+begin_src emacs-lisp
     ...
     (setq qq/preferred-fixed-font "PragmataPro-12")
     (setq qq/preferred-variable-family "CMU Sans Serif")
     (setq qq/preferred-variable-height 120)
   #+end_src
...
**** Fonts
   ...
   #+begin_src emacs-lisp
     (setq qq/preferred-fixed-font "PragmataPro-9")
   #+end_src
...
**** Fonts
   ...
   #+begin_src emacs-lisp
     (setq qq/preferred-fixed-font "PragmataPro-14")
     (setq qq/preferred-variable-height 140)
   #+end_src
...
*** My preferred font
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (set-frame-font qq/preferred-fixed-font t) ; set font for all frames
     (set-face-attribute 'default nil :font qq/preferred-fixed-font)
   #+END_SRC
...
*** Fixed/Variable Width Faces
   ...
   #+begin_src emacs-lisp
     (defun qq/set-font-to-fixed-width ()
       ...
       (interactive)
       (face-remap-add-relative 'default '(:font qq/preferred-fixed-font)))
     (defun qq/set-font-to-variable-width ()
       "Change font in current window to a variable-width font."
       (interactive)
       (face-remap-add-relative 'default '(:family qq/preferred-variable-family :height qq/preferred-variable-height)))
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (eval-after-load "org"
       '(mapc
         (lambda (face)
             (set-face-attribute face nil :font qq/preferred-fixed-font
...
#+begin_example
# Local Variables:
# eval: (add-hook 'after-save-hook 'qq/async-babel-tangle 'append 'local)
# End:
#+end_example
